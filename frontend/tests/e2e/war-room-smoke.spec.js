...
test('War Room lifecycle shell and modules remain interactive', async ({ page, request }) => {
  ...
  await page.getByTestId('open-run-details').click()
  await expect(page.getByTestId('run-details-drawer')).toBeVisible()
  await page.getByTestId('run-details-drawer').getByRole('button', { name: '关闭' }).click()
  await page.getByLabel('设置').click()
