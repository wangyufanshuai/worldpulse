...
def store_bytes(content: bytes, *, media_type: str = "text/markdown") -> tuple[str, int, str, str, bool]:
    """Store trusted application-generated bytes in the same content-addressed layout.

    The caller remains responsible for treating the source text as untrusted data.
    This helper only bypasses multipart handling, never the size, signature or path gates.
    """
    if media_type not in {"text/plain", "text/markdown", "text/csv"}:
        raise HTTPException(status_code=415, detail="Generated document media type is not supported")
    if not content:
        raise HTTPException(status_code=422, detail="Empty generated documents are not accepted")
    if len(content) > max_upload_bytes():
        raise HTTPException(status_code=413, detail=f"Document exceeds the {max_upload_bytes()} byte upload limit")
    _verify_signature(media_type, content[:16])
    ...
    if created:
        temporary = root / "tmp"
        temporary.mkdir(parents=True, exist_ok=True)
        temp_path = (temporary / f"generated-{uuid4().hex}.part").resolve()
        if root not in temp_path.parents:
            raise HTTPException(status_code=422, detail="Unsafe generated document path")
        try:
            with temp_path.open("xb") as handle:
            ...
            temp_path.unlink(missing_ok=True)
    return content_hash, len(content), blob_key, media_type, created
