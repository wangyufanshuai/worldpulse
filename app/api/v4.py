...
def evidence_sources(request: Request, status: str | None = None, source_type: str | None = None) -> list[EvidenceSource]:
    return evidence_registry.list_sources(status=status, source_type=source_type, organization_id=_organization(request))
...
def evidence_source_create(payload: EvidenceSourceCreate, request: Request) -> EvidenceSource:
    return evidence_registry.create_source(payload, _actor(request), organization_id=_organization(request))
...
def evidence_source_detail(source_id: str, request: Request) -> EvidenceSource:
    return evidence_registry.get_source(source_id, organization_id=_organization(request))
...
def evidence_snapshot_create(payload: EvidenceSnapshotCreate, request: Request) -> EvidenceSnapshot:
    return evidence_registry.create_snapshot(payload, _actor(request), organization_id=_organization(request))
...
def evidence_snapshot_detail(snapshot_id: str, request: Request) -> EvidenceSnapshot:
    return evidence_registry.get_snapshot(snapshot_id, organization_id=_organization(request))
...
def evidence_claim_create(payload: EvidenceClaimCreate, request: Request) -> EvidenceClaim:
    return evidence_registry.create_claim(payload, _actor(request), organization_id=_organization(request))
...
def evidence_claim_detail(claim_id: str, request: Request) -> EvidenceClaim:
    return evidence_registry.get_claim(claim_id, organization_id=_organization(request))
...
def evidence_pack_create(payload: EvidencePackCreate, request: Request) -> EvidencePack:
    return evidence_registry.create_pack(payload, _actor(request), organization_id=_organization(request))
...
def evidence_pack_detail(pack_id: str, request: Request) -> EvidencePack:
    return evidence_registry.get_pack(pack_id, organization_id=_organization(request))


@router.post("/evidence/calibration/sync")
def evidence_calibration_sync(request: Request) -> dict:
    return evidence_registry.sync_calibration_evidence(_actor(request), organization_id=_organization(request))
...
def project_evidence_sync(project_id: str, request: Request, run_id: str | None = None) -> EvidenceSyncResult:
    return evidence_registry.sync_project_evidence(project_id, _actor(request), run_id=run_id, organization_id=_organization(request))
