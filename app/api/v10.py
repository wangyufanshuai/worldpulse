...
def batches(organization_id: str, request: Request): return service.list_batches(organization_id)

@router.post("/organizations/{organization_id}/evaluations/standard", response_model=EvaluationBatch)
def standard(organization_id: str, payload: EvaluationBatchCreateRequest, request: Request): return service.create_standard_batch(organization_id, actor(request), payload)

@router.post("/organizations/{organization_id}/evaluations/observation", response_model=EvaluationBatch)
def observation(organization_id: str, payload: EvaluationBatchCreateRequest, request: Request): return service.create_batch(organization_id, actor(request), payload, source_type="observation")
...
def project_experiment(organization_id: str, project_id: str, draft_id: str, payload: EvaluationBatchCreateRequest, request: Request): return service.create_project_experiment(organization_id, project_id, draft_id, actor(request), payload)

@router.get("/evaluations/{batch_id}", response_model=EvaluationBatch)
def detail(batch_id: str): return service.get_batch(batch_id)

@router.get("/evaluations/{batch_id}/members", response_model=list[EvaluationMember])
def members(batch_id: str): return service.list_members(batch_id)

@router.get("/evaluations/{batch_id}/metrics", response_model=list[EvaluationMetric])
def metrics(batch_id: str): return service.list_metrics(batch_id)

@router.get("/evaluations/{batch_id}/events", response_model=list[dict])
def events(batch_id: str, after_seq: int = Query(default=0, ge=0)): return service.list_events(batch_id, after_seq)
...
def event_stream(batch_id: str, request: Request, after_seq: int = Query(default=0, ge=0)):
    ...
    def generate():
        ...
        while idle < 20:
            rows = service.list_events(batch_id, cursor)
            if rows:
                idle = 0
                for row in rows:
...
def report(batch_id: str): return service.report(batch_id)

@router.post("/evaluations/{batch_id}/pause", response_model=EvaluationBatch)
def pause(batch_id: str, request: Request): return service.control(batch_id, "pause")

@router.post("/evaluations/{batch_id}/resume", response_model=EvaluationBatch)
def resume(batch_id: str, request: Request): return service.control(batch_id, "resume")

@router.post("/evaluations/{batch_id}/cancel", response_model=EvaluationBatch)
def cancel(batch_id: str, request: Request): return service.control(batch_id, "cancel")

@router.post("/evaluations/{batch_id}/retry", response_model=EvaluationBatch)
def retry(batch_id: str, request: Request): return service.retry(batch_id, actor(request))
