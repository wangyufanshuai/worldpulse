...
| `app/services/run_lifecycle/repository.py` | API and Worker now use a Run Control application port; job/lease repository and projection helpers remain behind the compatibility adapter for the next slice | repository import surface | lifecycle, recovery and PostgreSQL tests |
