...
Status: active / Phase 3 Plugin SDK Slice 3A accepted; production plugin dispatch disabled; public V2 creation remains deterministic-only
...
authorized slice is 3B: wrap three existing read-only data paths behind
connector/extractor adapters while preserving existing cache, cutoff, fallback,
Evidence application-port and organization quota behavior.
